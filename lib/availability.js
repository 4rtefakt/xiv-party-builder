...
// Calcule la longueur de la "queue" d'un slot : nombre d'heures CONSECUTIVES
// dans le même jour, à la suite, où la densité reste >= threshold. Aide à
// départager des slots ex-æquo en privilégiant ceux qui permettent un raid
// plus long avec le même nombre de personnes.
function tailLength(analysis, day, hour, threshold) {
  ...
  for (let i = hourIdx + 1; i < analysis.hours.length; i++) {
    ...
    if (analysis.matrix[day][nextH].length >= threshold) tail++;
    else break; // chain rompue
  }
  return tail;
}
...
export function bestSlotWithTail(analysis) {
  ...
  // endHour : dernière heure (incluse) où la densité reste >= maxCount.
  // = hours[indexOf(start) + tail]. Utile pour afficher "20h → 22h" (range).
  const hourIdx = analysis.hours.indexOf(best.hour);
  best.endHour = hourIdx >= 0 ? analysis.hours[hourIdx + best.tail] : best.hour;
  return best;
}
