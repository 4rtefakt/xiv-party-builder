...
// Cap arbitraire généreux : un raid de 12h c'est extrême, mais qq'un peut
// vouloir bloquer une journée entière de farm. Pas la peine d'être plus strict.
export const MAX_SLOT_DURATION = 12;
...
export function cellInSlots(day, hour, slots) {
  ...
  for (const s of slots) {
    ...
    const dur = typeof s.duration === 'number' && s.duration >= 1 ? s.duration : 1;
...
export function slotsIntersect(a, b) {
  ...
  const dA = typeof a.duration === 'number' && a.duration >= 1 ? a.duration : 1;
  const dB = typeof b.duration === 'number' && b.duration >= 1 ? b.duration : 1;
