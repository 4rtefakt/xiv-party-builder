...
//   alliances    : raid24/chaotic — [{ name, results }] par alliance (prend
//                  le pas sur `result` : la compo est calculée PAR alliance
//                  côté caller, comme à l'écran, pas globalement)
//   t            : fonction i18n (caller-side)
export function buildMarkdown({
  contentLabel, raidWhen, players, bannedJobs, link, result, alliances, t
}) {
  const ROLE_LABELS = {
  ...
  // Pousse les sections par rôle d'un tableau de results ; renvoie les
  // non-assigné·es (banc) pour agrégation côté caller.
  function pushRoleSections(results) {
  ...
  const validAlliances = Array.isArray(alliances)
    ? alliances.filter(a => a && Array.isArray(a.results))
    : [];

  if (validAlliances.length > 0) {
    const bench = [];
    validAlliances.forEach((al, i) => {
      if (i > 0) lines.push('');
      lines.push(`**${al.name}**`);
      bench.push(...pushRoleSections(al.results));
    });
    if (bench.length > 0) {
      lines.push('');
    ...
    const bench = pushRoleSections(result.results);
    if (bench.length > 0) {
      lines.push('');
      lines.push(t('dc_bench', { names: bench.map(r => r.name).join(', ') }));
    }
