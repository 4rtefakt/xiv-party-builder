...
// Les préférences s'expriment en BUCKETS absolus : tier 0 = ★ Main,
// 1 = "Ça me va", 2 = "Si besoin". Le front cycle ces trois états au clic.
export const MAX_PREF_TIER = 2;

// Migration des rangs legacy (ancien modèle "ordre de clic" : tiers stricts
// 0..n-1, possiblement > 2) vers les buckets. Détection fiable : la nouvelle
// UI ne produit JAMAIS de tier > MAX_PREF_TIER, donc tout tier au-delà est
// legacy. Répartition proportionnelle par rang, groupes à tier égal gardés
// ensemble :
//   - mainCount     = max(1, round(n/3))  → le haut de liste devient ★ Main
//   - ifNeededCount = floor(n/4)          → le bas devient "Si besoin"
//   - le milieu     = "Ça me va"
// Ex : 4 picks → 1 Main / 2 Ça me va / 1 Si besoin ;
//      8 picks → 3 Main / 3 Ça me va / 2 Si besoin.
// Idempotent : une liste déjà en buckets (max ≤ 2) ressort inchangée.
// Renvoie un NOUVEAU tableau de tiers, parallèle à `preferences`.
export function bucketizePrefTiers(preferences, prefTiers) {
  const n = Array.isArray(preferences) ? preferences.length : 0;
  if (n === 0) return [];
  // Tiers effectifs : mêmes règles de fallback que getPrefTier (longueur
  // alignée + valeurs numériques, sinon ordre strict 0..n-1).
  const aligned = Array.isArray(prefTiers) && prefTiers.length === n
    && prefTiers.every(t => typeof t === 'number' && Number.isFinite(t));
  const eff = aligned ? prefTiers.slice() : preferences.map((_, i) => i);
  if (Math.max(...eff) <= MAX_PREF_TIER) return eff;

  const mainCount = Math.max(1, Math.round(n / 3));
  const ifNeededCount = Math.floor(n / 4);
  const bucketAt = pos => pos < mainCount ? 0 : (pos < n - ifNeededCount ? 1 : 2);

  // Entrées triées par tier croissant (stable) ; chaque groupe à tier égal
  // prend le bucket de sa PREMIÈRE position (les égalités restent égales).
  const order = eff.map((t, i) => [t, i]).sort((a, b) => a[0] - b[0] || a[1] - b[1]);
  const out = new Array(n);
  let groupStart = 0;
  for (let k = 0; k < order.length; k++) {
    if (k > 0 && order[k][0] !== order[k - 1][0]) groupStart = k;
    out[order[k][1]] = bucketAt(groupStart);
  }
  return out;
}
...
export function computeOptimalAssignment({
  ...
  const rosterPlayers = rawPlayers
    ...
    .map(p => {
      // Filtre les jobs bannis des prefs EN GARDANT prefTiers aligné : sans
      // ça, la longueur des tiers ne matche plus et getPrefTier retombe sur
      // l'index strict — bannir un job "promouvait" silencieusement les
      // prefs restantes (un tied-2e choix devenait 1er choix au scoring et
      // aux stats).
      const rawPrefs = Array.isArray(p.preferences) ? p.preferences : [];
      const hasTiers = Array.isArray(p.prefTiers) && p.prefTiers.length === rawPrefs.length;
      const preferences = [];
      const prefTiers = hasTiers ? [] : p.prefTiers;
      rawPrefs.forEach((id, i) => {
        if (banned.has(id)) return;
        preferences.push(id);
        if (hasTiers) prefTiers.push(p.prefTiers[i]);
      });
      return { ...p, preferences, prefTiers };
    });
...
export function buildResultFromAssignment(players, slots, assignment) {
  const results = players.map((p, i) => {
    const a = assignment[i];
    // rowId : identifiant stable de la ligne (quand le caller en fournit un).
    // Permet au front de retrouver le player du state sans matcher par nom
    // (ambigu en cas d'homonymes).
    if (a === null) {
      return { name: p.name, rowId: p.rowId || null, assigned: false, preferences: p.preferences, locked: false, lockedJob: p.lockedJob || null };
    }
    const job = JOB_BY_ID[a.jobId];
    // Rang AFFICHÉ = tier effectif (prefTiers), pas l'index brut : deux jobs
    // tied-1er (tiers [0,0]) doivent tous deux compter comme 1er choix dans
    // les stats et le badge de carte — c'est déjà ce que le solver score.
    const prefIdx = p.preferences.indexOf(a.jobId);
    const prefRank = prefIdx === -1 ? -1 : getPrefTier(p, prefIdx);
  ...
  const satisfaction = assigned.length === 0 ? 0 :
    Math.round(assigned.reduce((sum, r) => {
      ...
      // Clamp à 0 : avec les tiers "buckets" (absolus, trous permis), le
      // rang peut dépasser la taille de la liste (ex: un seul job coché en
      // bucket "Si besoin" = tier 2) — sans clamp la contribution devenait
      // négative et la satisfaction moyenne pouvait s'afficher < 0%.
      return sum + Math.max(0, 1 - cappedRank / Math.max(r.preferences.length, 1)) * 100;
    }, 0) / assigned.length);
