...
export const SCORING = {
  ...
  // distinct présent (max = 5 sous-rôles). 10 pts = ~1 rang de préférence,
  // assez pour départager des solutions proches mais pas pour surpasser
  // une vraie préférence forte (qui vaut ≥ 10 pts).
  SUBROLE_BONUS: 10
};
...
export function computeOptimalAssignment({
  ...
  if (players.length === 0) return { error: 'noPlayers' };
  ...
  const solutions = topSolutions.map(sol => buildResultFromAssignment(players, slots, sol.assignment));
