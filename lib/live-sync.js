...
export function createLiveSync({
  ...
  function connect() {
    ...
    // `socket` capture l'instance de CETTE connexion. Dans un vrai browser,
    // les events (close notamment) arrivent de façon ASYNCHRONE : après un
    // unsubscribe/re-subscribe rapide (changement de room), le close de
    // l'ancien socket peut arriver alors que `ws` pointe déjà sur le
    // nouveau. Sans le check `socket !== ws`, ce close périmé nullait le
    // socket courant et déclenchait un reconnect en doublon (deux WS vivants
    // → chaque save déclenchait deux callbacks 'changed').
    let socket;
    try {
      socket = ws = new WebSocketImpl(url);
    } catch (e) {
      scheduleReconnect();
      return;
    }
    ...
    socket.addEventListener('open', () => {
      if (socket !== ws) return; // connexion périmée
    ...
    socket.addEventListener('close', () => {
      if (socket !== ws) return; // close d'une connexion déjà remplacée
      clearTimers();
      ws = null;
