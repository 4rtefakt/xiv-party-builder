...
import { bucketizePrefTiers } from './scoring.js';
...
export function validateImportedPayload(payload) {
  ...
  const players = payload.p.map(raw => {
    ...
    // Migration : les rangs legacy "ordre de clic" (tiers stricts > 2, ou pt
    // absent avec 4+ picks) sont convertis en buckets ★ Main / Ça me va /
    // Si besoin à l'import. Idempotent — les données déjà en buckets passent
    // inchangées. Un salon mixte (anciennes lignes + nouvelles) devient donc
    // homogène dès le load.
    player.prefTiers = bucketizePrefTiers(player.preferences, player.prefTiers);
    if (raw.av !== undefined) {
  ...
  if (Array.isArray(payload.rs)) {
    for (const raw of payload.rs.slice(0, 7)) {
      ...
      // Duration en heures, multiples de 0.5, borne [0.5, 6]. Fallback à 1
      // si invalide. Les anciens salons (avant le passage à 6h max) qui
      // auraient stocké jusqu'à 12h voient leurs slots fallback à 1h.
      const dur = typeof raw.l === 'number' && Number.isFinite(raw.l)
        && raw.l >= 0.5 && raw.l <= 6 && (raw.l * 2) === Math.round(raw.l * 2)
        ? raw.l : 1;
...
export function encodePayload(s) {
  ...
  if (Array.isArray(s.raidSlots) && s.raidSlots.length > 0) {
    ...
    for (const slot of s.raidSlots.slice(0, 7)) {
      ...
      // Multiples de 0.5 dans [0.5, 6]. Cohérent avec validateImportedPayload
      // et la validation serveur dans functions/api/save.js.
      const dur = typeof slot.duration === 'number' && Number.isFinite(slot.duration)
        && slot.duration >= 0.5 && slot.duration <= 6 && (slot.duration * 2) === Math.round(slot.duration * 2)
        ? slot.duration : 1;
